...
export const newFrameView = (renderer) => ({
    colorAttachments: [{
        ...
        loadOp: "clear",
        clearValue: { r: (Math.sin(Date.now() / 1000)/2) + 0.5, g: 0.4, b: 0.4, a: 1 },
        storeOp: "store",
    }],
...
const createDepthTextureFromCanvas = (canvas) => gpu.createTexture({
    size: {
        width: canvas.width,
...
export function AllocateTextureArray(bitmaps) {
    ...
    const textureArray = gpu.createTexture(Object.assign({ size: [width, height, layers] }, bitmaps[0].textureFormat));
    ...
    return { view: textureArray.createView({ dimension: "2d-array", arrayLayerCount: layers}), width, height, layers, ppi: bitmaps[0].ppi};
}
...
export function AllocateMesh(mesh) {
    ...
    return Object.assign(createBuffer(mesh.verts, GPUBufferUsage.VERTEX), {vertCount: mesh.vertCount});
}
...
export function AllocateInstancedBuffer(sizePerInstance, count, strideOffset = 16) {
    strideOffset--; 
    return gpu.createBuffer({
